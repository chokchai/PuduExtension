...
(function() {
  var $addReplyButton, $bottomPagination, $comemntsBody, $commentsBox, $commentsContent, $commentsHeader, $commentsTable, $commentsTableInner, $content, $menu, $topPagination, commentHtml, emoHtml, focus, insertAtCaret, like, likes, _i, _len, _ref, _ref1, _ref2, _ref3;
...
<tr>\
  ...
  if ($addReplyButton.size() > 0) {
    $bottomPagination.after($(commentHtml).find('input[name="topicid"]').val($('input[name="topicid"]').val()).end().html());
    $('#emo div').click(function(event) {
  ...
  if ($commentsContent.size() > 0) {
    $commentsContent.each(function() {
      ...
      if (cIndex !== -1) {
        return $(this).html(html.substring(0, cIndex));
      }
    });
  }
