...
(function() {
  var $addReplyButton, $bottomPagination, $comemntsBody, $commentsBox, $commentsContent, $commentsHeader, $commentsTable, $commentsTableInner, $content, $menu, $topPagination, $torrentCommentLink, commentHtml, emoHtml, focus, insertAtCaret, like, likes, _i, _len, _ref, _ref1, _ref2, _ref3;
...
<li class="thumbnail"><div style="background-position: -1392px 0px;"  data-text=":yes:"  href="javascript:void(0);"></div></li>\
  ...
  commentHtml = function(action, hidden, textarea) {
    return '<div>\
...
<tr>\
  ...
  if ($addReplyButton.size() > 0) {
    $bottomPagination.after($(commentHtml('?action=post', 'topicid', 'body')).find('input[name="topicid"]').val($('input[name="topicid"]').val()).end().html());
  }

  if ($torrentCommentLink.size() > 0) {
    $('.pudu-comments-table+p').after($(commentHtml('comment.php?action=add', 'tid', 'text')).find('input[name="tid"]').val($('input[name="torrentid"]').val()).end().html());
  }
  ...
  if ($commentsBox.size() > 0) {
    ...
    for (_i = 0, _len = likes.length; _i < _len; _i++) {
      like = likes[_i];
      if ((10 >= (_ref = like.score) && _ref > 0)) {
        focus = 1;
      } else if ((20 >= (_ref1 = like.score) && _ref1 > 10)) {
        focus = 2;
      } else if ((30 >= (_ref2 = like.score) && _ref2 > 20)) {
        focus = 3;
      } else if ((40 >= (_ref3 = like.score) && _ref3 > 30)) {
        focus = 4;
      } else {
        focus = 5;
      }
